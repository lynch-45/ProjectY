...
app = FastAPI(
    ...
    version="1.0.0",
    description=(
        "Hackathon MVP: deterministic cascading-failure "
        "simulation over a synthetic city."
    )
)
...
def root():
    return {
        ...
        "message":
            "Synthetic-city hackathon MVP API"
    }
...
@app.post("/simulation/reverse")
def reverse_simulation(
    request: ReverseSimulationRequest
):

    if request.target_asset_id not in CITY:

        raise HTTPException(
            status_code=404,
            detail=(
                f"Unknown target asset: "
                f"{request.target_asset_id}"
            )
        )
